...
-- NOTE: You'll need actual user IDs from your auth.users and user_profiles tables
-- This script uses placeholder UUIDs that must be replaced with real data

-- For demonstration, we'll create sample data structure
-- In production, replace 'manager-user-id' and 'provider-user-id' with actual IDs

-- Sample: Get actual IDs (run these queries separately to get real IDs)
-- SELECT id, user_id, first_name, last_name, role FROM user_profiles WHERE role = 'manager' LIMIT 1;
-- SELECT id, user_id, first_name, last_name, role FROM user_profiles WHERE role = 'service_provider' LIMIT 1;

-- Then replace in the INSERT statements below:
-- {manager-profile-id} = actual manager user_profiles.id
-- {manager-user-id} = actual manager auth.users.id
-- {provider-profile-id} = actual provider user_profiles.id
-- {provider-user-id} = actual provider auth.users.id
-- {task-id} = actual task id to attach to

-- Example template (uncomment and update with real IDs to use):
/*

-- 1. Create sample checklist for a task
INSERT INTO public.task_checklists (task_id, title, description, is_required)
VALUES (
  '{task-id}',
  'Room Renovation Checklist',
  'Items to complete for room renovation task',
  true
);

-- Get the checklist ID
SELECT id INTO checklist_id FROM task_checklists WHERE task_id = '{task-id}' LIMIT 1;

-- 2. Add checklist items
INSERT INTO public.task_checklist_items (checklist_id, label, description, display_order)
VALUES
  (checklist_id, 'Paint walls', 'Apply 2 coats of paint', 1),
  (checklist_id, 'Install fixtures', 'Install new light fixtures', 2),
  (checklist_id, 'Replace flooring', 'Install new tile flooring', 3),
  (checklist_id, 'Final inspection', 'Quality check and cleanup', 4);

-- 3. Create evidence requirements
INSERT INTO public.task_evidence_requirements (task_id, required_evidence_types, description)
VALUES (
  '{task-id}',
  ARRAY['photo', 'video'],
  'Provider must submit photos and video showing completed work'
);

-- 4. Create a task report when provider starts work
INSERT INTO public.task_reports (task_id, provider_id, status, description, percentage_complete, last_updated_by)
VALUES (
  '{task-id}',
  '{provider-profile-id}',
  'in_progress',
  'Started work on room renovation. Walls are being prepared for painting.',
  25,
  '{provider-user-id}'
);

-- 5. Mark some checklist items as completed
INSERT INTO public.task_report_checklist_items (report_id, checklist_item_id, is_completed, completed_at)
SELECT tr.id, tci.id, true, now()
FROM task_reports tr
JOIN task_checklists tc ON tc.task_id = tr.task_id
JOIN task_checklist_items tci ON tci.checklist_id = tc.id
WHERE tr.task_id = '{task-id}' AND tci.label IN ('Paint walls', 'Install fixtures');

-- 6. Create sample issue
INSERT INTO public.task_issues (task_id, provider_id, title, description, severity, status)
VALUES (
  '{task-id}',
  '{provider-profile-id}',
  'Material Delay',
  'The special paint ordered has been delayed. Expected delivery is 2 days late.',
  'high',
  'open'
);
...
-- ============================================================================
-- 11. NOTIFICATION TYPE ADDITIONS (for notifications table)
-- ============================================================================
-- The notifications type enum should be extended. If you need to add these types:
-- Update the check constraint on notifications.type to include:
-- 'report_submitted', 'evidence_approved', 'issue_raised', 'issue_resolved'

-- You can verify and update with:
-- ALTER TABLE public.notifications
-- ADD CONSTRAINT notifications_type_check_v2 CHECK (
--   type::text = ANY (array[
--     'complaint_filed'::text,
--     'complaint_acknowledged'::text,
--     'task_created'::text,
--     'task_updated'::text,
--     'task_assigned'::text,
--     'task_accepted'::text,
...
--
-- NEXT STEPS:
-- 1. Get real user IDs from your database
-- 2. Uncomment and modify the sample data section with real IDs
-- 3. Add TypeScript interfaces to client code
